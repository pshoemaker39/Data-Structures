...
public class A3SimplerBST<K extends Comparable<? super K>, V> {
    ...
    public int twoChildrenCount() {
        int counter = 0;
        return twoChildrenCount(root, counter);

    }

    private int twoChildrenCount(Node<K, V> x, Integer counter) {

        if (x != null) {
            counter += twoChildrenCount(x.left, counter) + twoChildrenCount(x.right, counter);
    ...
    public int sameValueCount(V target) {
        int counter = 0;
        return sameValueCount(root, counter, target);
    }

    private int sameValueCount(Node<K, V> x, Integer counter, V target) {

        if(x != null) {
            counter += sameValueCount(x.left, counter, target) + sameValueCount(x.right, counter, target);

            //StdOut.println(x.key+" "+x.val+" "+target);

            if (x.val.equals(target)) {
                //StdOut.println(" Counter Inc.: " + x.key + " " + x.val + " " + target);
                counter += 1;
            }
        }
