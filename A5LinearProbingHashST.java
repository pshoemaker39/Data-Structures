...
/* ***********************************************************************
 *  Compilation:  javac A5LinearProbingHashST.java
 *  Execution:    java A5LinearProbingHashST
 *
 *  Symbol table implementation with linear probing hash table.
 *
 *  % java A5LinearProbingHashST
 *  128.112.136.11
 *  208.216.181.15
 *  null
 *
...
public class A5LinearProbingHashST<K, V> {
	...
	public int clusterCount() {
		int counter = 0;
		int max = 0;
		A5LinearProbingHashST<Integer, Integer> clusters = new A5LinearProbingHashST<>();

		for(K k : keys()) {
			if(clusters.contains(hash(k))){
				int count = clusters.get(hash(k));
				clusters.put(hash(k), count+1);
				if(max < count) {
					max = count;
				}

			} else {
				clusters.put(hash(k),1);
				counter +=1;
